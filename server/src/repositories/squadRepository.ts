...
export class MemorySquadRepository implements SquadRepository {
  ...
  async swapPlayers(participantId: string, input: SwapPlayersInput, fixtures?: FixtureSeed[]): Promise<SwapResultSummary> {
    const squad = await this.getOrCreate(participantId)
    const now = this.now()
    ...
    const openWindow = getOpenSwapWindow(now, fixtures)
    ...
    const window = assertSwapAllowed({
      ...
      nationsInvolved: [inPlayer.teamCode, outPlayer.teamCode],
      swapsUsedInWindow,
      now,
      fixtures,
    })
...
export class PostgresSquadRepository implements SquadRepository {
  ...
  async getOrCreate(participantId: string) {
    ...
    const slotResult = await this.pool.query<{
      ...
      `
        SELECT s.slot_key, s.slot_group, s.slot_class, p.player_id, p.display_name,
               COALESCE(ts.team_code, p.nationality_code) AS team_code,
               p.nationality_code, p.rating, p.position_codes, p.position_main, p.image_url
        FROM squad_slots s
  ...
  async swapPlayers(participantId: string, input: SwapPlayersInput, fixtures?: FixtureSeed[]): Promise<SwapResultSummary> {
    ...
    try {
      ...
      // Current squad slots (the lock-time composition) keyed by player, with team code + codes.
      const slotRows = await client.query<{
        ...
        `
          SELECT s.slot_key, s.slot_group, s.slot_class, s.player_id,
                 COALESCE(ts.team_code, p.nationality_code) AS team_code,
                 s.position_codes
          FROM squad_slots s
          JOIN world_cup_players p ON p.player_id = s.player_id
      ...
      const teamCodeByPlayer = new Map<number, string>()
      for (const row of slotRows.rows) {
        teamCodeByPlayer.set(Number(row.player_id), row.team_code ?? '')
      }
      const inTeamCode = teamCodeByPlayer.get(input.playerInId)
      const outTeamCode = teamCodeByPlayer.get(input.playerOutId)
      if (inTeamCode === undefined || outTeamCode === undefined) {
        throw new SquadValidationError('Both players must be in your squad to swap them.')
      }
      ...
      const window = assertSwapAllowed({
        ...
        nationsInvolved: [inTeamCode, outTeamCode],
        swapsUsedInWindow,
        now,
        fixtures,
      })
