...
export function createParticipantRouter(
  ...
  router.post('/squad/swap', async (req, res) => {
    ...
    try {
      const result = await squadRepository.swapPlayers(participant.participantId, parsed)
  ...
  router.get('/squad/swaps', async (_req, res) => {
    const participantId = res.locals.participant.participantId as string
    const history = await squadRepository.listSwaps(participantId)
    ...
    res.json({
      history,
      windows,
      openWindow: getOpenSwapWindow(now),
      swapsUsedByWindow,
      hardStopAt: swapHardStopEpoch(),
      hasHardStopPassed: hasSwapHardStopPassed(now),
      currentLineup,
    })
  })
