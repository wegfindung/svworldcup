...
type ValueCopy = AppMessages['playerValue']
...
// The value is the headline; keep two decimals so cheap-vs-pricey differences stay legible.
    ...
    <article className="grid gap-4 rounded-[1rem] border border-white/8 bg-black/18 p-4 transition hover:border-[var(--color-accent)]/28 hover:bg-white/5 lg:grid-cols-[minmax(0,1.4fr)_minmax(11rem,0.7fr)_minmax(9rem,0.5fr)] lg:items-center">
      ...
      <div>
        <div className="flex items-end gap-2">
          <p className="text-3xl font-black leading-none text-[var(--color-accent)]">{formatValue(value)}</p>
          <p className="mono pb-1 text-[10px] uppercase tracking-[0.18em] text-[var(--color-muted)]">{copy.unitLabel}</p>
        </div>
        <p className="mono mt-1.5 text-[11px] text-[var(--color-muted)]">
          {formatPoints(points)} {ptsLabel} · {copy.costLabel} {formatCost(player.capCost)}
        </p>
      </div>
...
export function ValueStatsPanel({ locale }: { locale: LocaleCode }) {
  const copy = getMessages(locale).playerValue
  ...
  const [mode, setMode] = useState<ValueMode>('total')
  ...
  function selectMode(next: ValueMode) {
    setMode(next)
    setPage(1)
  }
      ...
      <section className="glass-panel rounded-[1.15rem] p-4">
        <div className="grid gap-3 lg:grid-cols-[minmax(10rem,1fr)_auto_auto] lg:items-center">
          ...
          <div className="flex flex-wrap gap-1 rounded-full border border-white/8 bg-black/18 p-1">
            {(['total', 'perGame'] as ValueMode[]).map((option) => (
              <button
                key={option}
                type="button"
