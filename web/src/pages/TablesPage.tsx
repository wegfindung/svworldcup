...
// Floating "back to top" control for the long participant/nation boards. Portaled to document.body so the
// fixed positioning is relative to the viewport — `<main>` keeps a lingering `reveal-in` transform, which
// would otherwise make a `fixed` child position against that ancestor instead of the viewport. The parent
// only mounts this on the nations/rookie/veteran tabs (not the internally-scrolled country-picks table).
