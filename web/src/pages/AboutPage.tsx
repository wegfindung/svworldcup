...
const englishCopy: AboutCopy = {
  ...
  intro:
    'The Grand Tournament Community Event is a free, fan-made fantasy game for the 2026 World Cup, built by members of the Soccerverse community and powered by Soccerverse player data. It is not an official Soccerverse product — it is made by players, for players, to bring the community together around the tournament.',
  ...
  svBody:
    'Soccerverse is an online football management game where you run real clubs, build squads, and can hold influence in players and clubs across a living football economy. This event maps real World Cup performances onto Soccerverse players, so the same names you follow in the game move the leaderboards here. If you are new, the links below are the best places to start; if you already play, they are your shortcuts back into the game and the wider community.',
...
const spanishCopy: AboutCopy = {
  ...
  svBody:
    'Soccerverse es un juego online de gestión futbolística donde diriges clubes reales, formas plantillas y puedes tener influencia en jugadores y clubes dentro de una economía futbolística viva. Este evento traslada las actuaciones reales del Mundial a los jugadores de Soccerverse, así que los mismos nombres que sigues en el juego mueven las clasificaciones aquí. Si eres nuevo, los enlaces de abajo son el mejor punto de partida; si ya juegas, son tus atajos de vuelta al juego y a la comunidad.',
...
const italianCopy: AboutCopy = {
  ...
  svBody:
    'Soccerverse è un gioco manageriale di calcio online in cui gestisci club reali, costruisci rose e puoi detenere influenza su giocatori e club in un’economia calcistica viva. Questo evento riporta le prestazioni reali del Mondiale sui giocatori di Soccerverse, così gli stessi nomi che segui nel gioco muovono le classifiche qui. Se sei nuovo, i link qui sotto sono il punto di partenza migliore; se già giochi, sono le tue scorciatoie verso il gioco e la community.',
...
const frenchCopy: AboutCopy = {
  ...
  intro:
    'Le Grand Tournament Community Event est un jeu de fantasy gratuit et amateur pour la Coupe du monde 2026, créé par des membres de la communauté Soccerverse et propulsé par les données de joueurs Soccerverse. Ce n’est pas un produit officiel Soccerverse : il est fait par des joueurs, pour des joueurs, afin de rassembler la communauté autour du tournoi.',
  ...
  svBody:
    'Soccerverse est un jeu de gestion de football en ligne où vous dirigez de vrais clubs, composez des effectifs et pouvez détenir de l’influence sur des joueurs et des clubs dans une économie footballistique vivante. Cet événement reporte les performances réelles de la Coupe du monde sur les joueurs Soccerverse : les mêmes noms que vous suivez dans le jeu font bouger les classements ici. Si vous débutez, les liens ci-dessous sont le meilleur point de départ ; si vous jouez déjà, ce sont vos raccourcis vers le jeu et la communauté.',
...
const portugueseCopy: AboutCopy = {
  ...
  svBody:
    'O Soccerverse é um jogo de gestão de futebol online onde geres clubes reais, montas plantéis e podes ter influência em jogadores e clubes numa economia de futebol viva. Este evento transpõe os desempenhos reais do Mundial para os jogadores do Soccerverse, por isso os mesmos nomes que segues no jogo movem as classificações aqui. Se és novo, os links abaixo são o melhor ponto de partida; se já jogas, são os teus atalhos de volta ao jogo e à comunidade.',
  officialEyebrow: 'soccerverse oficial',
...
const russianCopy: AboutCopy = {
  ...
  intro:
    'Grand Tournament Community Event — это бесплатная фанатская фэнтези-игра к чемпионату мира 2026, созданная участниками сообщества Soccerverse и работающая на данных игроков Soccerverse. Это не официальный продукт Soccerverse — он сделан игроками для игроков, чтобы объединить сообщество вокруг турнира.',
...
const chineseCopy: AboutCopy = {
  ...
  intro:
    'Grand Tournament Community Event 是一个面向 2026 年世界杯的免费、粉丝制作的梦幻游戏，由 Soccerverse 社区成员打造，并使用 Soccerverse 球员数据。它不是 Soccerverse 的官方产品——它由玩家为玩家而制作，让社区围绕这届赛事聚在一起。',
  ctaRegister: '注册你的阵容',
  ctaRules: '阅读规则',
  svEyebrow: '什么是 soccerverse',
  svTitle: '本活动运行所依托的足球世界',
  svBody:
    'Soccerverse 是一款在线足球经理游戏，你可以经营真实球队、组建阵容，并在一个活跃的足球经济中持有球员和俱乐部的影响力。本活动将真实的世界杯表现映射到 Soccerverse 球员上，因此你在游戏中关注的那些名字也会在这里牵动榜单。如果你是新手，下面的链接是最好的起点；如果你已经在玩，它们是你回到游戏和更广泛社区的快捷方式。',
...
const japaneseCopy: AboutCopy = {
  ...
  intro:
    'Grand Tournament Community Event は、2026 年ワールドカップ向けの無料・ファンメイドのファンタジーゲームで、Soccerverse コミュニティのメンバーによって作られ、Soccerverse の選手データで動いています。Soccerverse の公式製品ではありません——プレイヤーによる、プレイヤーのためのもので、大会を中心にコミュニティをつなぐために作られました。',
  ctaRegister: 'スカッドを登録する',
  ctaRules: 'ルールを読む',
  svEyebrow: 'soccerverse とは',
  svTitle: 'このイベントが動く土台となるサッカーの世界',
  svBody:
    'Soccerverse はオンラインのサッカーマネジメントゲームで、実在のクラブを運営し、スカッドを組み、生きたサッカー経済の中で選手やクラブへの影響力を保有できます。このイベントは実際のワールドカップの成績を Soccerverse の選手にマッピングするため、ゲームで追っている名前がそのままここのランキングを動かします。初めての方は下のリンクが最良の出発点です。すでにプレイしている方には、ゲームや広いコミュニティへの近道です。',
